...
| `logs` | `from src import logs` | 普通业务模块统一使用的项目 Loguru logger；使用 f-string 生成以日志技术 owner 的唯一状态符号开头、包含稳定 `key=value` 上下文且符号与 level 匹配的消息，并仅在恢复或任务终止边界记录 exception。CLI 配置后写 stderr；Flask 服务配置后写 system file 和 stderr；job 子进程的 stderr 由 `JobRuntime` 捕获。 | 在业务模块中调用 `remove()`/`add()`、记录敏感值、重复记录同一异常堆栈或拥有 sink 和 job 文件生命周期。 | `logs.info(f"✅ request; request_id={request_id}")` |
...
| `configure_cli_logging` | `from src.utils.logger import configure_cli_logging` | 仅由 CLI composition root 调用一次，清除 Loguru 默认 sink，并把公共 `src.logs` 的 `INFO` 及以上日志路由到带完整模块名和行号、但不重复 function 的 stderr。 | 普通业务模块调用、创建 file sink、拥有 job 文件或 Flask system log。 | `configure_cli_logging()` |
...
| `ObjectPaths` | `from src.utils.path import ObjectPaths` | 表示同一规范 processed、feature 或 label 对象的不可变 payload/Meta 路径对，字段为 `payload_path` 和 `meta_path`；实例由 `PathManager` 的对象路径方法返回。 | Raw source-native payload、文件 I/O、对象可用性、schema、metadata/lineage 或独立拼接路径。 | `paths = pm.processed_object(dataset_name="daily_bar", version="v1", trade_date="2026-07-20"); payload = paths.payload_path` |
| `PathManager` | `from src.utils.path import PathManager` | 绑定调用方提供的已存在绝对 `pathlib.Path` root，通过只读 `storage_root` 属性暴露该规范 root，创建 raw、staging、processed、features、labels、experiments 六个固定顶层目录，并从完整正式身份返回单日或年度规范路径；规范文件名固定的 processed、feature 和 label 对象一次返回 `ObjectPaths`；`require_safe_basename` 校验单一路径段，`require_experiment_id` 校验 CLI/Job experiment ID。 | 环境变量读取、创建 storage root 或业务分区、文件 I/O、数据可用性、schema、metadata/lineage、任意路径拼接、未定义 experiment artifact。 | `pm = PathManager(storage_root); calendar = pm.processed_year_object(dataset_name="trade_calendar", version="v1", calendar_year=2026)` |
...
public symbol。`configure_cli_logging` 与 `configure_system_logging` 分别只供 CLI 与
Flask 服务 composition root 使用；普通模块不得直接配置 Loguru sinks。
